...
IF EXISTS (SELECT 1 FROM dbo.hishopping_category WHERE name = N'食品')
   AND NOT EXISTS (SELECT 1 FROM dbo.hishopping_category WHERE name = N'食品生鲜')
BEGIN
    UPDATE dbo.hishopping_category
    SET name = N'食品生鲜', icon_text = N'鲜', description = N'零食、粮油、生鲜、速食、地方特产', sort_no = 1
    WHERE name = N'食品';
END
GO

IF EXISTS (SELECT 1 FROM dbo.hishopping_category WHERE name = N'图书')
   AND NOT EXISTS (SELECT 1 FROM dbo.hishopping_category WHERE name = N'图书文娱')
BEGIN
    UPDATE dbo.hishopping_category
    SET name = N'图书文娱', icon_text = N'书', description = N'图书、文具、影音、文创与娱乐周边', sort_no = 15
    WHERE name = N'图书';
END
GO
...
USING (VALUES
    (N'食品生鲜', N'鲜', N'零食、粮油、生鲜、速食、地方特产', 1),
    (N'酒水饮料', N'饮', N'饮料、茶咖、冲调、酒水等', 2),
    (N'家用电器', N'电', N'厨房电器、生活电器、大家电、小家电', 3),
    (N'手机数码', N'数', N'手机、平板、相机、智能设备、数码配件', 4),
    (N'电脑办公', N'办', N'电脑整机、办公设备、文具耗材、网络设备', 5),
    (N'家居家装', N'居', N'装修建材、灯具照明、收纳装饰、居家日用', 6),
    (N'家具家纺', N'家', N'家具、床品、布艺、家纺套件', 7),
    (N'服饰内衣', N'服', N'男女服饰、内衣、配饰、季节服装', 8),
    (N'鞋靴箱包', N'包', N'鞋靴、箱包、旅行用品、皮具配件', 9),
    (N'美妆个护', N'美', N'护肤、彩妆、洗护、香氛和个人护理', 10),
    (N'母婴童装', N'婴', N'母婴用品、童装童鞋、喂养洗护、启蒙用品', 11),
    (N'运动户外', N'运', N'运动装备、户外用品、健身器材、骑行用品', 12),
    (N'玩具乐器', N'玩', N'儿童玩具、模型、乐器、益智用品', 13),
    (N'汽车用品', N'车', N'车载电器、养护用品、汽车装饰、配件工具', 14),
    (N'图书文娱', N'书', N'图书、文具、影音、文创与娱乐周边', 15),
    (N'宠物用品', N'宠', N'宠物食品、清洁护理、牵引用具、玩具用品', 16),
    (N'珠宝饰品', N'饰', N'珠宝首饰、手表配饰、礼品饰品', 17),
    (N'鲜花绿植', N'花', N'鲜花、绿植、园艺花材、节庆花礼', 18),
    (N'厨具餐具', N'厨', N'锅具、餐具、厨房工具、烘焙器具', 19),
    (N'清洁纸品', N'洁', N'纸品湿巾、衣物清洁、家清工具、除菌用品', 20),
    (N'医疗健康', N'健', N'健康监测、护理用品、营养保健与康复辅助', 21),
    (N'工业五金', N'五', N'五金工具、工业耗材、仪器仪表、劳保用品', 22),
    (N'农资园艺', N'农', N'种子肥料、园艺工具、农用物资、绿化用品', 23),
    (N'本地生活', N'生', N'到店服务、生活服务、家政维修、休闲体验', 24),
    (N'充值缴费', N'充', N'话费充值、流量包、生活缴费与账户充值', 25),
    (N'虚拟商品', N'虚', N'虚拟权益、电子凭证、在线服务和非实物商品', 26),
    (N'游戏道具', N'游', N'游戏点卡、道具礼包、皮肤兑换和虚拟装备', 27),
    (N'数字内容', N'数', N'电子书、音视频内容、素材模板和数字资料', 28),
    (N'会员服务', N'会', N'会员权益、订阅服务、增值服务和专属礼包', 29),
    (N'课程教育', N'课', N'在线课程、学习资料、考试培训和技能提升', 30),
    (N'软件工具', N'软', N'软件授权、效率工具、插件模板和云服务', 31),
    (N'票务卡券', N'票', N'演出票券、兑换卡、礼品卡和消费券', 32),
    (N'二手闲置', N'二', N'二手数码、闲置好物、转让商品和旧物循环', 33),
    (N'其他商品', N'其', N'暂未归入固定类目的综合商品', 99)
...
USING (
    ...
    FROM (VALUES
    (N'食品生鲜', N'德运高钙全脂成人牛奶粉', N'来自澳大利亚天然牧场，口感醇厚。', N'德运高钙全脂成人牛奶粉来自澳大利亚天然牧场，奶香浓郁，适合早餐、烘焙和日常营养补充。', 100.00, 128.00, 4.8, 1360, 100, N'营养', N'assets/img/catalog/food1.jpg', N'linear-gradient(135deg,#f59e0b,#fbbf24)', N'奶', N'原味,加钙,全脂', N'单罐装,双罐装,家庭装'),
    (N'食品生鲜', N'瑞士莲软心进口巧克力', N'精选多种口味，入口丝滑。', N'瑞士莲软心进口巧克力精选多种经典口味，外壳细腻，内馅柔滑，适合作为零食、礼物和下午茶搭配。', 50.00, 69.00, 4.7, 2200, 100, N'甜品', N'assets/img/catalog/food2.jpg', N'linear-gradient(135deg,#7c2d12,#f97316)', N'巧', N'牛奶味,榛仁味,黑巧味', N'分享装,礼盒装,家庭装'),
    ...
    (N'图书文娱', N'《楚乔传》全三册', N'正版包邮，全三册经典文学读本。', N'《楚乔传》全三册收录完整故事篇章，适合休闲阅读、小说收藏和礼品赠送。', 50.00, 68.00, 4.9, 980, 100, N'文学', N'assets/img/catalog/book1.jpg', N'linear-gradient(135deg,#92400e,#d97706)', N'传', N'平装,精装,收藏版', N'单套,两套,礼盒套装'),
    (N'图书文娱', N'红楼梦', N'中国古代四大名著之一。', N'《红楼梦》是中国古代四大名著之一，人物众多、结构宏大，适合课外阅读和文学鉴赏。', 60.00, 88.00, 4.9, 1680, 100, N'名著', N'assets/img/catalog/book2.jpg', N'linear-gradient(135deg,#dc2626,#f97316)', N'红', N'平装,精装,注释版', N'单本,上下册,阅读套装'),
    (N'图书文娱', N'三国演义', N'阅读古典名著，品味经典文化。', N'《三国演义》讲述群雄逐鹿与英雄谋略，是中国古典章回小说代表作。', 45.00, 69.00, 4.8, 1510, 100, N'经典', N'assets/img/catalog/book3.jpg', N'linear-gradient(135deg,#1f2937,#ef4444)', N'三', N'平装,精装,青少版', N'单本,上下册,名著套装'),
    (N'图书文娱', N'我不乱发脾气', N'3-6 岁亲子共读启蒙图书。', N'《我不乱发脾气》面向 3-6 岁儿童，通过故事引导孩子识别情绪、表达情绪，适合亲子共读。', 45.00, 58.00, 4.7, 880, 100, N'童书', N'assets/img/catalog/book4.jpg', N'linear-gradient(135deg,#22c55e,#06b6d4)', N'童', N'绘本版,注音版,礼盒版', N'单本,三本套装,亲子套装'),
    ...
    (N'食品生鲜', N'蒙牛特仑苏有机纯牛奶', N'250ml*24，家庭早餐常备。', N'蒙牛特仑苏有机纯牛奶规格为 250ml*24，奶味香醇，适合早餐、加餐和家庭囤货。', 150.00, 188.00, 4.8, 2460, 100, N'有机', N'assets/img/catalog/food3.jpg', N'linear-gradient(135deg,#38bdf8,#f8fafc)', N'奶', N'有机纯牛奶,低脂牛奶,梦幻盖', N'12盒,24盒,家庭箱'),
    (N'食品生鲜', N'楼兰蜜语红枣', N'500g*2袋，新疆特产。', N'楼兰蜜语红枣来自新疆产区，果肉饱满，甜度自然，适合直接食用、煲汤、泡茶和办公室零食。', 45.00, 59.00, 4.7, 1830, 100, N'特产', N'assets/img/catalog/food4.jpg', N'linear-gradient(135deg,#b91c1c,#f59e0b)', N'枣', N'原味,夹核桃,礼盒装', N'500g,500g*2袋,节日礼盒'),
    (N'手机数码', N'无线降噪蓝牙耳机', N'低延迟连接，通勤学习都轻便。', N'无线降噪蓝牙耳机支持低延迟模式、触控操作和长续航充电盒，适合通勤、学习和游戏语音。', 199.00, 299.00, 4.6, 640, 100, N'数码', N'assets/img/product-earbuds.svg', N'linear-gradient(135deg,#4f46e5,#06b6d4)', N'耳', N'白色,黑色,紫色', N'标准版,降噪版,游戏版'),
    (N'服饰内衣', N'四季基础纯棉T恤', N'柔软透气，日常百搭。', N'四季基础纯棉T恤采用亲肤棉面料，版型简洁，适合作为通勤、校园和居家日常穿搭。', 59.00, 89.00, 4.5, 520, 100, N'百搭', N'assets/img/product-shoes.svg', N'linear-gradient(135deg,#ec4899,#8b5cf6)', N'衣', N'白色,黑色,灰色', N'S,M,L,XL'),
    (N'游戏道具', N'手游皮肤兑换礼包', N'电子兑换码，适合课程演示虚拟商品。', N'手游皮肤兑换礼包为演示用虚拟商品，购买后可作为电子兑换码类商品展示，适合测试游戏道具分类和皮肤关键词搜索。', 30.00, 45.00, 4.8, 420, 100, N'皮肤', N'assets/img/product-watch.svg', N'linear-gradient(135deg,#7c3aed,#db2777)', N'游', N'经典款,限定款,主题款', N'单份,三份礼包,组合礼包'),
    (N'虚拟商品', N'云端资料包兑换卡', N'非实物电子凭证，适合在线交付。', N'云端资料包兑换卡为演示用虚拟商品，可用于模拟电子凭证、在线权益和非实物商品搜索展示。', 19.90, 29.90, 4.7, 360, 100, N'虚拟', N'assets/img/product-notebook.svg', N'linear-gradient(135deg,#0ea5e9,#6366f1)', N'虚', N'基础包,进阶包,团队包', N'月卡,季卡,年卡'),
    (N'课程教育', N'Java Web 入门课程资料', N'Servlet、JSP 与 SQL Server 学习包。', N'Java Web 入门课程资料包含 Servlet、JSP、SQL Server 项目练习说明和示例素材，用于课程教育类商品展示。', 49.00, 99.00, 4.9, 280, 100, N'课程', N'assets/img/catalog/book1.jpg', N'linear-gradient(135deg,#16a34a,#0f766e)', N'课', N'基础版,练习版,项目版', N'单人版,小组版,班级版')
